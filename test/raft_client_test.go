...
// func TestSyncTwoClientsSameFileLeaderFailure_backup(t *testing.T) {
...
func TestSyncTwoClientsSameFileLeaderFailure(t *testing.T) {
	t.Logf("leader1 gets a request. leader1 gets a request  ")
	cfgPath := "./config_files/3nodes.txt"
	test := InitTest(cfgPath)
	defer EndTest(test)
	fmt.Println("test1")
	...
	err = worker1.UpdateFile(file2, "changed text")
	if err != nil {
		t.FailNow()
	}

	//client1 syncs with small change
	err = SyncClient("localhost:8080", "test0", BLOCK_SIZE, cfgPath)
	if err != nil {
		t.Fatalf("Sync failed")
	}
	...
	fmt.Println("-----------------------Internal state of leader 0 ")
	test.Clients[0].GetInternalState(test.Context, &emptypb.Empty{})
